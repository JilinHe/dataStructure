...
class BoardWidget extends Pad {
    ...
    private void mouseReleased(Square s) {
        ...
        if (_currentSq.size() == 2) {
            Square temp1 = _currentSq.get(0);
            Square temp2 = _currentSq.get(1);
            if (_board.get(temp1) == _board.turn()) {
                Move move = Move.mv(temp1, temp2);
                if (_board.isLegal(move)) {
                    _commands.offer(temp1.toString()
                            + "-" + temp2.toString());
                }
            }
            _currentSq.clear();
    ...
    private List<Square> _currentSq = new ArrayList<Square>();
    /** Store the pressed square. */
    private Square _press;
}
