...
public class NFA {
    ...
    private class State {
        ...
        public Set<State> successors(char c) {
            if (_edges.containsKey(c)) {
                Set<State> succses = this._edges.get(c);
                if (c == EPSILON) {
                    Set<State> temps = new HashSet<State>(succses);
                    for (State temp1: succses) {
                        temps.addAll(temp1.successors(EPSILON));
                    }
                    return temps;
                } else {
                    return succses;
    ...
    public boolean matches(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            c = c == '*' ? EPSILON : c;
            for (State temp1: stateSet) {
                new_stateSet.addAll(temp1.successors(c));
            }
            stateSet.clear();
            stateSet.addAll(new_stateSet);
            new_stateSet.clear();
            for (State temp3: stateSet) {
                if (temp3._accepting && i == s.length() - 1) {
                    return true;
                }
                new_stateSet.addAll(temp3.successors(EPSILON));
            }
            for (State temp5: new_stateSet) {
                if (temp5._accepting && i == s.length() - 1) {
                    return true;
                }
            }
            stateSet.clear();
